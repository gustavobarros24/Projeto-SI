import { CheckCircle, TrendingUp, MessageSquare, Stethoscope, Award, CircleDot, XCircle } from "lucide-react"
...
const GRADE_STYLE: Record<XRayFinalReport["grade"], { color: string; stroke: string; bg: string }> = {
...
export function XRayFinalReportPanel({ report }: { report: XRayFinalReport }) {
  const { t } = useTranslation()
  const style = GRADE_STYLE[report.grade] ?? GRADE_STYLE["F"]
  const gradeLabel = t(`report.grades.${report.grade}`)

  const correctNum = (report.correct_findings ?? []).length
  const totalNum = report.ground_truth_findings_num ?? (report.ground_truth_findings ?? []).length
  const findingsPct = totalNum > 0 ? Math.round((correctNum / totalNum) * 100) : 0
          ...
          <Card>
            <CardHeader className="pb-1">
              <CardTitle className="flex items-center gap-1.5 text-sm">
                ...
                {t("report.correct_impression")}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <span className="block w-full rounded-md bg-muted px-3 py-1.5 text-sm font-semibold">
                {report.correct_impression}
              </span>
            </CardContent>
          </Card>
        </div>

        {/* coluna direita — achados */}
        <Card className="flex flex-col">
          <CardHeader className="pb-2 shrink-0">
            <CardTitle className="text-sm">
              {t("report.findings")} — {correctNum}/{totalNum}
            </CardTitle>
          ...
          <CardContent className="flex-1 overflow-y-auto">
            <ul className="space-y-2">
              {(report.ground_truth_findings ?? []).map((finding) => {
                const found = (report.correct_findings ?? []).some(
                  (f) => f.toLowerCase() === finding.toLowerCase()
                )
                return (
                  <li key={finding} className="flex items-center gap-2 text-sm">
                    {found
                      ? <CheckCircle className="h-4 w-4 text-emerald-500 shrink-0" />
                      : <CircleDot className="h-4 w-4 text-muted-foreground/40 shrink-0" />
                    }
                    <span className={found ? "" : "text-muted-foreground"}>{finding}</span>
                  </li>
                )
              })}
              {(report.incorrect_findings ?? []).length > 0 && (
                <>
                  <li className="border-t pt-2 mt-1">
                    <span className="text-xs font-medium text-red-500">{t("report.incorrect_findings")}</span>
                  </li>
                  {(report.incorrect_findings ?? []).map((finding) => (
                    <li key={finding} className="flex items-center gap-2 text-sm">
